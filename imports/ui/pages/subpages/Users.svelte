...
<!-- CONTAINER -->
<div class="dark:bg-dark-700 bg-dark-50 h-[calc(100%_-_40px)] flex text-xs">
    <!-- LEFT -->   
    {#if bLeftPanel}
        <div class="grid grid-rows-[50px,1fr] shadow-lg basis-[300px]">
            ...
            <div class="flex flex-col p-10 gap-6">
                ...
                <div>
                    <button class="bg-alfa-600 shadow uppercase  text-white  rounded w-full h-8" on:click="{fGetAllUsers}"> Listar usuarios</button>
                </div>
                <div>
                    <button class="bg-alfa-600 shadow uppercase  text-white  rounded w-full h-8" on:click="{fToggleDrawer}"> Registrar nuevo usuario</button>
                </div>
            </div>
            <!-- OPTIONS_END -->
        </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="text-dark-600 dark:text-dark-200 dark:bg-dark-800 h-9 border-b border-dark-200 dark:border-dark-600">
                            ...
                            <td class="px-5">
                                <button class="bg-beta-700 shadow text-white  rounded h-7 min-w-[60px]" on:click="{_=>fRemoveUser(user._id)}">Eliminar</button>
                            </td>
                        </tr>
                        {/each}
                    </tbody>
...
<!-- DRAWER -->
{#if bDrawer}
    <div class="dark:bg-dark-700 text-xs bg-dark-50 shadow-lg absolute top-[40px] bottom-0 z-40 w-[300px] grid grid-rows-[50px,1fr]">
        <!-- BAR -->
        <div class="flex justify-end px-4 items-center"> 
            <button class="h-6 w-6 shadow rounded-full text-white  hover:ring" on:click={_=> bDrawer = false}>
        ...
        <form class="flex flex-col p-10 gap-6" on:submit|preventDefault={fCreateUser}>
            <div>
                <h1 class="text-dark-700 dark:text-dark-100 font-bold ">REGISTRAR USUARIO</h1>
            </div>
            <div class="grid grid-rows-[auto,auto] gap-1">
                <label for="firstname" class="text-dark-700 dark:text-dark-300 ">Nombre:</label>
                <input bind:value="{firstname}" type="text" name="firstname" class="h-8  ring-1 ring-dark-300 rounded px-3 dark:text-dark-800 focus:outline-none focus:ring-alfa-500" required>
            </div>

            <div class="grid grid-rows-[auto,auto] gap-1">
                <label for="lastname" class="text-dark-700 dark:text-dark-300 ">Apellido:</label>
                <input bind:value="{lastname}" type="text" name="lastname" class="h-8  ring-1 ring-dark-300 rounded px-3 dark:text-dark-800 focus:outline-none focus:ring-alfa-500" required>
            </div>
            <div class="grid grid-rows-[auto,auto] gap-1">
                <label for="_username" class="text-dark-700 dark:text-dark-300 ">Nombre de usuario:</label>
                <input bind:value="{username}" type="text" name="_username" class="h-8  ring-1 ring-dark-300 rounded px-3 dark:text-dark-800 focus:outline-none focus:ring-alfa-500" required>
            </div>
            <div class="grid grid-rows-[auto,auto] gap-1">
                <label for="_password" class="text-dark-700 dark:text-dark-300 ">Contraseña:</label>
                <input bind:value="{password}" type="text" name="_password" class="h-8  ring-1 ring-dark-300 rounded px-3 dark:text-dark-800 focus:outline-none focus:ring-alfa-500" required>
            </div>
            <div class="">
                <button class="bg-alfa-600 shadow uppercase text-white  rounded w-full h-8 mt-2" type="submit">
                    Registrar
                </button>
            </div>
        </form>
